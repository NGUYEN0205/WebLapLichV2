...
export const AnalyticsTab: React.FC<AnalyticsTabProps> = ({
    ...
    return (
      <div className="flex-1 p-4 md:p-8 flex flex-col items-center justify-center text-center bg-brand-bg/95">
        <div className="bg-brand-surface-medium border border-brand-border p-6 md:p-8 rounded-2xl md:rounded-3xl max-w-sm md:max-w-md shadow-2xl flex flex-col items-center gap-4">
          <BarChart2 className="w-12 h-12 md:w-16 md:h-16 text-brand-primary opacity-50 animate-bounce" />
          <h2 className="text-lg md:text-xl font-bold text-brand-text">Chưa có dữ liệu phân tích</h2>
  ...
  return (
    <div className="flex-1 p-3 md:p-6 overflow-y-auto bg-brand-bg/95 flex flex-col gap-4 md:gap-6 pb-20 lg:pb-6 ios-scroll">
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        {/* KPI 1 */}
        <div className="glass-panel p-3 md:p-4 rounded-2xl flex flex-col justify-between min-h-[110px]">
          <div className="flex justify-between items-start text-brand-on-surface-variant text-[10px] md:text-xs font-semibold uppercase tracking-wider">
            <span>Tổng môn học</span>
            <Award className="w-5 h-5 text-brand-primary" />
          </div>
          <div className="mt-2 text-2xl md:text-3xl font-extrabold text-brand-primary">
        ...
        <div className="glass-panel p-3 md:p-4 rounded-2xl flex flex-col justify-between min-h-[110px]">
          ...
          <div className="mt-2 text-2xl md:text-3xl font-extrabold text-brand-secondary">
        ...
        <div className="glass-panel p-3 md:p-4 rounded-2xl flex flex-col justify-between min-h-[110px]">
          ...
          <div className="mt-2 text-2xl md:text-3xl font-extrabold text-brand-tertiary">
          ...
          <p className="text-[10px] text-brand-on-surface-variant/90 mt-1 line-clamp-1">
            {freeDaysList.length > 0 ? `Thảnh thơi: ${freeDaysList.join(", ")}` : "Không có ngày nghỉ"}
          </p>
        </div>
        ...
        <div className="glass-panel p-3 md:p-4 rounded-2xl flex flex-col justify-between min-h-[110px]">
          ...
          <div className="mt-2 text-xl md:text-2xl font-extrabold text-red-450">
            {heaviestDay}
          </div>
          <p className="text-[10px] text-brand-on-surface-variant/70 mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="lg:col-span-2 glass-panel p-4 md:p-5 rounded-2xl flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
            <h3 className="font-bold text-xs md:text-sm text-brand-text uppercase tracking-wider">
              Phân bố số tiết học theo ngày
            </h3>
            <span className="text-[10px] bg-brand-surface-highest text-brand-text-muted px-2.5 py-1 rounded-md border border-brand-border self-start sm:self-auto">
              {averagePeriodsPerStudyDay} tiết / ngày học trung bình
            </span>
          </div>

          <div className="overflow-x-auto -mx-1">
            <div className="min-w-[320px] flex flex-col gap-4 mt-2">
              {Object.keys(DAYS_MAP).map((dayKeyString) => {
                ...
                return (
                  <div key={d} className="grid grid-cols-12 items-center gap-2">
                    <span className="col-span-2 text-xs font-semibold text-brand-text-muted">
                      {DAYS_MAP[d]}
                    </span>
                    
                    {/* Styled multi-tier bar graph */}
                    <div className="col-span-8 bg-brand-surface-highest/50 h-5 rounded-md overflow-hidden relative border border-brand-border/40 flex">
                      {academicCount > 0 && (
                        <div
                      ...
                      {busyCount > 0 && (
                        <div
                          ...
                          className="bg-brand-surface-highest/80 border-l border-dashed border-brand-border/60 h-full transition-all duration-500"
                    ...
                    <span className="col-span-2 text-xs font-mono text-right text-brand-primary whitespace-nowrap">
                      {academicCount > 0 ? `${academicCount}t` : ""}
                      {busyCount > 0 ? ` (+${busyCount}b)` : ""}
                      {totalCount === 0 ? "Nghỉ" : ""}
                    </span>
                  </div>
                );
          ...
          {/* Chart Legends */}
          <div className="flex flex-wrap gap-4 mt-3 text-[10px] bg-brand-surface-low/30 p-2 rounded-xl">
            <div className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded bg-brand-primary" />
              <span className="text-brand-text-muted">Giờ học chính khóa</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded bg-brand-surface-highest border border-dashed border-brand-border/60" />
              <span className="text-brand-text-muted">Lịch bận tự thiết lập cố định</span>
            </div>
          </div>
        </div>

        {/* Suggestion evaluation block */}
        <div className="glass-panel p-4 md:p-5 rounded-2xl flex flex-col gap-3.5">
          <h3 className="font-bold text-xs md:text-sm text-brand-text uppercase tracking-wider flex items-center gap-1.5">
          ...
          <div className="flex flex-col gap-3.5 mt-1.5">
            {/* Rule 1: rest days */}
            <div className="p-3 bg-brand-surface-low/50 rounded-xl border border-brand-border/20">
              ...
              <p className="text-xs text-brand-on-surface-variant">
                Lịch trình hiện có <strong className="text-brand-text">{freeDaysList.length} ngày nghỉ</strong> trong tuần. Cho phép bạn dễ dàng tự nghiên cứu bài tập lớn và gia tăng số buổi làm thêm kiếm thu nhập thụ động!
              </p>
            </div>
            ...
            <div className="p-3 bg-brand-surface-low/50 rounded-xl border border-brand-border/20">
              ...
              <p className="text-xs text-brand-on-surface-variant">
                Lượng thời gian học trung bình ở mức <strong className="text-brand-text">{averagePeriodsPerStudyDay} tiết/ngày</strong>. Đây là mật độ lý tưởng nhất (khoảng 3-4 tiếng/ngày), hạn chế mệt mỏi và chán nản trên giảng đường!
              </p>
            </div>

            {/* Hint alert */}
            <div className="text-[10px] text-brand-text-muted/70 leading-relaxed border-t border-brand-border/20 pt-2.5">
