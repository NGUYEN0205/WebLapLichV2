...
- **ClassOption (Lớp Học phần):** Thành phần cụ thể mô tả thời gian tổ chức học của một nhóm môn học nhất định.
  ...
  export interface ClassOption {
    ...
    room?: string;
    teacher?: string;
    isPinned?: boolean; // THÊM MỚI: đánh dấu ghim học phần bắt buộc
  }
  ```
- **Subject (Môn học):**
...
### 3.2 Lược đồ Thuật toán DFS cải tiến tích hợp Ràng buộc Ghim (Pin Constraint)
...
3. **Thực thi DFS Đệ quy:**
   ...
   - **Xử lý lựa chọn lớp của môn học tương ứng (Ghim ưu tiên):**
     - Tìm xem môn học hiện tại có lớp học phần nào đã được ghim (`isPinned`) hay không.
     - Nếu có các lớp học phần được ghim, danh sách tìm kiếm thử nghiệm (`choices`) sẽ giới hạn **chỉ chứa các lớp được ghim**.
     - Nếu không có lớp học phần nào được ghim, danh sách thử nghiệm vẫn là toàn bộ các phương án lớp học phần như ban đầu.
   - **Vòng lặp nhánh tuyển chọn trên miền thu hẹp:** Với môn học thứ `subjectIndex`, duyệt qua từng `ClassOption` trong tập `choices` đã lọc:
     - Kiểm tra nếu lớp học phần này bị **đè giờ** lên bất kỳ lớp học nào đã được chọn trước đó trong mảng `currentClasses`. Nếu có xung đột đè giờ, **ngay lập tức tỉa nhánh (Prune branch)**.
     - Kiểm tra nếu lớp học phần này bị **đè giờ** lên bất kỳ hoạt động bận cá nhân nào trong `busyActivities`.
     - **Nguyên lý khoan dung bận đối với môn bị Ghim:**
       - Nếu có xung đột với thời gian bận cá nhân mà lớp học phần này **không** được ghim → tỉa nhánh (Prune branch).
       - Nếu có xung đột với thời gian bận cá nhân nhưng lớp học phần này **được** ghim → **không tỉa nhánh**; tiếp tục lấn sâu đệ quy, đồng thời lưu dấu để sinh ra cảnh báo đỏ (Warnings) trên bảng điều khiển giao diện, giúp sinh viên linh hoạt xử lý (ví dụ: xin nghỉ làm hôm đó để đi học).
     - Nếu lớp học phần thỏa mãn các bước kiểm tra: Gọi đệ quy duyệt môn học tiếp theo: `dfsSearch(subjectIndex + 1, [...currentClasses, lớp học hiện tại])`.
...
## 5. Quản lý Chủ đề Giao diện (Theme Management)

Hệ thống quản lý chủ đề sáng/tối đồng bộ sử dụng các biến CSS Custom Properties kết hợp với Tailwind CSS v4:
